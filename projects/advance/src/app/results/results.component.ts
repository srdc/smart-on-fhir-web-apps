...
export class ResultsComponent implements OnInit, OnDestroy {
  ...
  applySuggestions() {
    ...
    const prefetch = CdsUtils.stateToPrefetch(state, this.advanceService.conceptDefinitions, <fhir4.Patient>this.patient, true)
    this.suggestions.forEach(card => card.suggestions.forEach((suggestion: any) => {
      if (suggestion.apply) {
        ...
        const definition = this.advanceService.conceptDefinitions.find((_definition: any) =>
